...
const int mxN = 607;
vector<int> augmented_matrix[mxN];
...
{
    bool t = false;
    for(int j = 0;(int)augmented_matrix[i].size() > j + 1;j++)
    {
    ...
    if(!t && augmented_matrix[i].back() != 0) return false;
    return true;
}
void solveLinearSystem()
{
    int numberOfEquations,numberOfVariables;
    cin >> numberOfEquations >> numberOfVariables;
    ...
    vector<int> val(numberOfVariables);
