...
struct SegmentTree
{
    struct Node
    {
        ...
        int right_sum;
        int whole_sum;
        Node(int _ss,int _ls,int _rs,int _ws)
        {
            subarray_sum = _ss, left_sum = _ls, right_sum = _rs, whole_sum = _ws;
        } 
        Node()
        {
            subarray_sum = 0, left_sum = 0, right_sum = 0, whole_sum = 0;
        }
    };
    vector<Node> tree;
    SegmentTree(int _sz)
    ...
    Node query(int l,int r,int tl,int tr,int pos = 1)
    {
        ...
        int mid = (l + r) / 2;
        return F(query(l, mid, tl, tr, pos * 2), query(mid + 1, r, tl, tr, pos * 2 + 1));
    }
};
void solve()
{
    ...
    while(q--)
    {
        ...
        cin >> x >> y;
        x--,y--;
        cout << st.query(0, n - 1, x, y).subarray_sum << endl;
    }
}
int32_t main()
{
